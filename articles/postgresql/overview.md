...
Azure Database for PostgreSQL flexible server is a fully managed PostgreSQL database service on Azure. It gives you granular control and flexibility over database management functions and configuration settings, so you can tailor the service to your requirements.

You can customize server configuration for your workload. The architecture lets you collocate the database engine with the client tier for lower latency and choose high availability within a single availability zone and across multiple availability zones. An Azure Database for PostgreSQL flexible server instance also provides cost optimization controls with the ability to stop and start your server and a burstable compute tier that's ideal for workloads that don't need full compute capacity continuously. The service supports various major community versions of PostgreSQL. For details on the specific versions supported, see [Supported versions of PostgreSQL in Azure Database for PostgreSQL](configure-maintain/concepts-supported-versions.md). The service is available in various [Azure regions](https://azure.microsoft.com/global-infrastructure/services/).
